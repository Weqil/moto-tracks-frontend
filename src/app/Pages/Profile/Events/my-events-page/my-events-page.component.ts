...
export class MyEventsPageComponent implements OnInit {
  ...
  setFilterInTape(filter: 'all' | 'current' | 'expired' | 'rgp') {
    ...
    this.loadingService.showLoading().then((res: any) => (loader = res))
    this.getEvents()
  ...
  getEvents(): Observable<any> {
    ...
    return this.eventService.getAllEvents({ ...this.eventsFilter, userOnlyAppointment: Number(this.userRider) }).pipe(
      tap((res: any) => {
  ...
  finishEvents(): Observable<any> {
    return this.eventService
      .getAllEvents({
        userId: String(this.userService.user.value?.id),
        dateEnd: moment().subtract(7, 'days').locale('ru').format('YYYY-MM-DD'),
        sortField: 'date_start',
        sort: 'asc',
      })
      .pipe(
  ...
  ionViewWillEnter() {
    this.loadingService.observableLoaderScoup([this.getEvents(), this.finishEvents()]).subscribe()
  }

...
export class CreateEventsPageComponent implements OnInit {
  ...
  stepInvalidate() {
    if (this.raceTypeSelectedItem.name == 'Согласована (обычная)') {
      ...
      if (
        this.createEventForm.value.name.length <= 3 ||
        this.createEventForm.value.region.length <= 3 ||
        this.createEventForm.value.dateStart.length <= 3
        // || !this.currentComission.length
      ) {
        return true
      } else {
  ...
  getAllGroups() {
    this.groupService
      .getAllGroup({ userId: this.userService.user.value?.id })
      .pipe()
      .subscribe((res: any) => {
        this.userGroups = res.grades
      })
    this.groupService
      .getAllGroup({ gradeNotParent: 1 })
      .pipe()
      .subscribe((res: any) => {
  ...
  createNewGroup() {
    if (!this.createClassesForm.invalid) {
      this.loadingService.showLoading()
      this.groupService
        .createGroup(this.createClassesForm.value)
        .pipe(
          finalize(() => {
            this.loadingService.hideLoading()
          }),
  ...
  submitForm() {
    if (!this.stepInvalidate()) {
      ...
      this.eventService
        .createEvent(createEventFormData)
        .pipe(
          finalize(() => {
            this.loadingService.hideLoading()
          }),
        ...
        .subscribe((res: any) => {
          ...
          this.userService
            .addComission(
              race.id,
              this.currentComission.map((user) => user.id),
            )
            .pipe(
              catchError((error) => {

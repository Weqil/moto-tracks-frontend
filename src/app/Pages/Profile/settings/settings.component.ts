...
export class SettingsComponent implements OnInit {
  ...
  editEmail() {
    ...
    this.userService
      .editUser(this.personalSettingsForm.value)
      .pipe(
        catchError((error) => {
      ...
      .subscribe((res: any) => {
        ...
        this.userService.refreshUser(() => {
          this.personalViewForm.patchValue({
            emailView: this.personalSettingsForm.get('email')?.value,
            phoneView: this.personalSettingsForm.get('phone'),
          })
  ...
  navigateInAgreed() {
    this.closeModalAgreedNotVerificated()
    setTimeout(() => {
  ...
  private confirmDeleteAccount() {
    this.loading.showLoading().then((loader) => {
      this.userService
        .deleteUser()
        .pipe(finalize(() => this.loading.hideLoading(loader)))
        .subscribe({
          next: () => {
